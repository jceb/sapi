...
export const HTTPMethod = $.NullaryType("HTTPMethod")(
  ...
  typeof x === "string" &&
  ["get", "post", "push", "put"].includes(x.toLowerCase)
);
...
export const fetchURL = (initAttrs) =>
  (urlPair) => {
    const _fetch = (init) =>
      (method) =>
        (resource) => F.attemptP(() => fetch(resource, { ...init, method }));
...
const toJSON = F.encaseP((r) => r.json());

// listOfRemainingPageIndexes :: Response -> Array(Integer)
const listOfRemainingPageIndexes = (r) =>
  S.range(2)(
    S.fromMaybe(1)(
      S.parseInt(10)(r.headers.get("x-total-pages")),
    ),
  );

// drainURL :: StrMap -> Pair(HTTP_Method_String)(URL_String) -> Future(Response)
const drainURL = (initAttrs) =>
  (urlPair) =>
    S.pipe([
      fetchURL(initAttrs),
      S.chain(
...
export const gitLabFetch = (fetch_one) =>
  (token) =>
    (urlPair) => {
